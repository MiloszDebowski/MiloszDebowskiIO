...
class DebowskiPizzaBuilder : public PizzaBuilder {
public:
    
    void buildDough() override
    {
        pizza.setDough("Extra Thin Dough");
    }
    void buildSauce() override
    {
        pizza.setSauce("Jalapeno Sauce");
    }
    void buildTopping() override
    {
        pizza.setTopping("Pepperoni,Jalapenos, Oils, Slaska sausage");
    }
    Pizza getPizza() const override { return pizza; }
private:
    Pizza pizza;
...
cout<<endl<<endl;
    DebowskiPizzaBuilder DebowskiBuilder;
    cook.makePizza(DebowskiBuilder);
    Pizza Debowski = hawaiianBuilder.getPizza();
    Debowski.displayPizza();
    return 0;
}
